...
df.imputed <- transform(df, steps = ifelse(is.na(steps), sapply(df$day.interval, average.for.day.interval), steps))
```
...
h2 <- ggplot(day.total2, aes(x=steps)) + 
      geom_histogram(binwidth=1000, colour="black", fill="red") + 
      xlab("Total number of steps per day") +
      geom_vline(xintercept=meanTotalSteps2, color="blue", size=1.5) +
      geom_vline(xintercept=medianTotalSteps2, color="green", linetype="dashed", size=1.5) + 
